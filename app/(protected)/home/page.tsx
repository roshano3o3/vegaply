  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width:8,height:8,background:"#080d1a",border:`1px solid ${color}33`,
  ...
  return (
    <div className={`job-card${hot&&earlyBirdMode?" job-card-hot":""}`} style={{display:"flex",flexDirection:"column",gap:12,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:10,display:"flex",gap:5,flexWrap:"wrap"}}>
        ...
        <button className={`action-card-btn interview-btn${job.interview?" done":""}`} onClick={e => { e.stopPropagation(); onInterview(); }} disabled={job.interviewLoading} title="AI interview questions">
          {job.interviewLoading ? <><div className="spin-sm"/>Loading…</> : job.interview ? "✓ Prep" : "🤖 Prep"}
        </button>
        <button className={`action-card-btn cover-btn${job.coverLetter?" done":""}`} onClick={e => { e.stopPropagation(); onCoverLetter(); }} disabled={job.coverLetterLoading} title="AI cover letter">
        ...
        <button className={`action-card-btn track-btn${isTracked?" tracked":""}`} onClick={e => { e.stopPropagation(); onTrack(); }} title="Add to tracker">
          {isTracked ? "✓" : "+ Track"}
        </button>
      </div>
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        :root{
          --bg:#04060f;--bg2:#080d1a;--bg3:#0c1220;--bg4:#111827;
          --v:#7c3aed;--v2:#6d28d9;--c:#06b6d4;--c2:#0891b2;
          --g:#10b981;--a:#f59e0b;--r:#ef4444;--pk:#ec4899;
          --border:rgba(124,58,237,0.15);--border2:rgba(255,255,255,0.07);
          --text:#f1f5f9;--text2:#94a3b8;--text3:#475569;
        }
        [data-theme="light"]{
          --bg:#f8fafc;--bg2:#fff;--bg3:#f1f5f9;--bg4:#e2e8f0;
          --border:rgba(124,58,237,0.2);--border2:rgba(0,0,0,0.08);
        ...
        .job-card-hot:hover{border-color:rgba(245,158,11,0.4)!important;box-shadow:0 12px 40px rgba(0,0,0,0.4),0 0 20px rgba(245,158,11,0.07)!important}
        ...
        .apply-btn-hot:hover{box-shadow:0 8px 24px rgba(245,158,11,0.35)!important}
        ...
        .page-btn:hover:not(.active):not(:disabled){background:rgba(255,255,255,0.04);color:var(--text2)}
      ...
      <div className="app-layout">
        ...
        <aside className="sidebar">
          <div className="sidebar-card">
            ...
            <ResumePanel
              ...
              onResume={async (t,n) => {
                ...
                const strength = calcResumeStrength(t);
                ...
                {isMatching && (
                  <div style={{marginTop:7}}>
                    <div style={{height:2,background:"rgba(255,255,255,0.05)",borderRadius:2,overflow:"hidden"}}>
                      <div style={{height:"100%",background:"linear-gradient(90deg,var(--v),var(--c))",borderRadius:2,width:`${matchProgress}%`,transition:"width .3s"}}/>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="sidebar-card">
            ...
            <div className="salary-range-display">
              <span>${salaryMin}k</span><span>→ ${salaryMax}k</span>
            </div>
        ...
        <main className="content">
          ...
          {activeTab==="tracker" && (
            <TrackerView apps={trackedApps}
              onUpdateStatus={(id,s) => setTrackedApps(prev => prev.map(a => a.id===id?{...a,status:s}:a))}
              onUpdateNotes={(id,n) => setTrackedApps(prev => prev.map(a => a.id===id?{...a,notes:n}:a))}
              onRemove={id => setTrackedApps(prev => prev.filter(a => a.id!==id))}/>
          )}
          {activeTab==="analytics" && (
            ...
            <>
              {activeTab==="earlybird" && earlyBirdJobs.length>0 && !ebLoading && (
                <div className="eb-banner">
                  <div>
                    <div style={{fontSize:14,fontWeight:700,color:var(--a),marginBottom:2}}>⚡ Early Bird Mode Active</div>
                    <div style={{fontSize:11,color:"rgba(245,158,11,0.4)"}}>Jobs posted in the last 24 hours — minimal competition</div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:18}}>
                    <div style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:800,color:var(--a)}}>{earlyBirdJobs.length}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.25)"}}>Fresh Jobs</div></div>
                    <div style={{width:1,height:28,background:"rgba(255,255,255,0.06)"}}/>
                    <div style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:800,color:var(--r)}}>{hotCount}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.25)"}}>🔥 Under 6h</div></div>
                  </div>
                </div>
              )}

              {autoOpenDone && (
                <div style={{background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.15)",borderRadius:10,padding:"11px 14px",marginBottom:14,fontSize:12,fontWeight:600,color:"#10b981",display:"flex",alignItems:"center",gap:8}}>
                  🚀 Opened top 3 matches in new tabs!
                </div>
              )}
              ...
              {!currentLoading && paginatedJobs.length > 0 && (
                <>
                  <div style={{fontSize:11,color:"var(--text3)",marginBottom:14}}>
                    ...
                    {isEbMode && <span style={{color:var(--a),fontWeight:600}}> · ⚡ All posted today</span>}
                  </div>
                  <div className="jobs-grid">
          ...
          <div className="ob-card">
            ...
            {onboardStep===1 && (
              <>
                ...
                <div className="ob-sub">The AI-powered job search platform built for students. Let's set you up in 60 seconds.</div>
                ...
                <div className="chips-wrap">
                  {roleChips.map(r => (
                    <div key={r} className={`chip${onboardRole===r?" sel":""}`} onClick={() => setOnboardRole(onboardRole===r?"":r)}>{r}</div>
                  ))}
                </div>
                ...
                <div className="ob-btns">
                  <button className="btn-cont" style={{flex:1}} onClick={() => { if(onboardRole.trim()) setOnboardStep(2); }}>Continue →</button>
                </div>
              </>
            )}
            ...
            {onboardStep===2 && (
              <>
                ...
                <div className="chips-wrap">
                  {locChips.map(l => (
                    <div key={l} className={`chip${onboardLocation===l?" loc-sel":""}`} onClick={() => setOnboardLocation(onboardLocation===l?"":l)}>{l}</div>
                  ))}
                </div>
                <div className="or-div">or type your own</div>
                ...
                <div className="ob-btns">
                  <button className="btn-back" onClick={() => setOnboardStep(1)}>← Back</button>
                  <button className="btn-cont" onClick={() => { if(onboardLocation.trim()) setOnboardStep(3); }}>Continue →</button>
                </div>
              </>
            )}
              ...
              <>
                ...
                <input id="ob-file-input" type="file" accept=".pdf" style={{display:"none"}} onChange={async e => {
                  ...
                  try {
                    ...
                    if (text.trim()) {
                      setResumeText(text); setResumeFileName(file.name);
                      const strength = calcResumeStrength(text); setResumeStrength(strength);
                      lsSet("applysmart_resume",text); lsSet("applysmart_resume_name",file.name);
                      const{supabase}=await import("@/lib/supabase");
                      const{data:{user}}=await supabase.auth.getUser();
                      if(user) await supabase.from("resumes").insert({user_id:user.id,title:"Resume",file_name:file.name,resume_text:text});
                    }
                  } catch(err){console.error(err);}
                ...
                <div className="upload-zone" onClick={() => document.getElementById("ob-file-input")?.click()}>
                  ...
                  <div style={{fontSize:14,fontWeight:600,color:"rgba(255,255,255,0.5)",marginBottom:4}}>
                    {onboardParsing?"Parsing…":"Drop your resume PDF here"}
                  </div>
        ...
        <div onClick={() => setShowResumeHistory(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:24,backdropFilter:"blur(12px)"}}>
          <div onClick={e => e.stopPropagation()} style={{background:"#07091a",border:"1px solid rgba(124,58,237,0.12)",borderRadius:16,padding:24,width:"100%",maxWidth:460,maxHeight:"78vh",overflowY:"auto"}}>
            ...
            {resumeHistory.length===0
              ? <p style={{color:"rgba(255,255,255,0.25)",textAlign:"center",padding:"24px 0",fontSize:12}}>No resumes saved yet</p>
              : resumeHistory.map((r,i) => (
                <div key={r.id} style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${i===0?"rgba(124,58,237,0.25)":"rgba(255,255,255,0.07)"}`,borderRadius:10,padding:14,marginBottom:9,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:12,fontWeight:600,color:"#fff",marginBottom:3}}>{r.file_name}</div>
                    ...
                    {i===0 && <div style={{fontSize:9,color:"#a78bfa",fontWeight:700,marginTop:3,letterSpacing:"0.3px"}}>ACTIVE</div>}
                  </div>
                  <button onClick={() => { setResumeText(r.resume_text); setResumeFileName(r.file_name); setResumeStrength(calcResumeStrength(r.resume_text)); lsSet("applysmart_resume",r.resume_text); lsSet("applysmart_resume_name",r.file_name); setShowResumeHistory(false); }}
                    style={{background:i===0?"rgba(124,58,237,0.08)":"linear-gradient(135deg,var(--v),var(--c))",color:i===0?"#a78bfa":"#fff",border:i===0?"1px solid rgba(124,58,237,0.25)":"none",borderRadius:7,padding:"7px 14px",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
                    {i===0?"Active":"Use This"}
                  </button>
                </div>
              ))
